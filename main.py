...
async def join_emote_and_leave(teamcode, uids, emote_code):
    """Join team, send emote, then leave after 0.1 second"""
    global whisper_writer, online_writer, key, iv, region
    
    try:
        print(f"[JEL] 🚀 Starting: Join team {teamcode} → Emote {len(uids)} player(s) → Leave after 0.1s")
        ...
        for uid in uids:
            try:
                ...
                if online_writer:
                    ...
                    # ⚡ 0.1 SECOND DELAY AFTER EACH EMOTE
                    await asyncio.sleep(0.1)
        ...
        # STEP 3: WAIT 0.1 SECOND BEFORE LEAVE
        await asyncio.sleep(0.1)  # ⚡ CHANGED FROM 1.0 TO 0.1
        
        # STEP 4: LEAVE TEAM (same as /solo command)
        if online_writer:
            leave_packet = await ExiT(None, key, iv)
            await SEndPacKeT(whisper_writer, online_writer, 'OnLine', leave_packet)
            print(f"[JEL] 👋 Left team after emote (0.1s delay)")
        else:
